...
from ..base.modules import LayerNorm2d
...
class ConvNeXtEncoder(ConvNeXt, EncoderMixin):
    def __init__(self, out_channels, depth=6, **kwargs):
        ...
        self.extra_stem_1 = nn.Sequential(
            nn.Conv2d(3, out_channels[1]//2, 3, 1, 1),
            LayerNorm2d(out_channels[1]//2),
            nn.GELU(),
            nn.Conv2d(out_channels[1]//2, out_channels[1], 3, 1, 1),
        )
        
        self.extra_stem_2 = nn.Sequential(
            nn.Conv2d(out_channels[1], out_channels[2]//2, 3, 2, 1),
            LayerNorm2d(out_channels[2]//2),
            nn.GELU(),
            nn.Conv2d(out_channels[2]//2, out_channels[2], 3, 1, 1),
        )
    ...
    def forward(self, x):
        ...
        features = []
        features.append(x)
        x_ = self.extra_stem_1(x)
        features.append(x_)
        x_ = self.extra_stem_2(x_)
        features.append(x_)
...
timm_convnext_encoders = {
    "convnext_atto": {
        ...
        "params": {
            "out_channels": (3, 10, 20, 40, 80, 160, 320),
            "depths": (2, 2, 6, 2),
            "dims": (40, 80, 160, 320),
            "conv_mlp": True
        },
    ...
    "convnext_femto": {
        ...
        "params": {
            "out_channels": (3, 12, 24, 48, 96, 192, 384),
            "depths": (2, 2, 6, 2),
            "dims": (48, 96, 192, 384),
            "conv_mlp": True
        },
    ...
    "convnext_pico": {
        ...
        "params": {
            "out_channels": (3, 16, 32, 64, 128, 256, 512),
            "depths": (2, 2, 6, 2),
            "dims": (64, 128, 256, 512),
            "conv_mlp": True
        },
    ...
    "convnext_nano": {
        ...
        "params": {
            "out_channels": (3, 20, 40, 80, 160, 320, 640),
            "depths": (2, 2, 8, 2),
            "dims": (80, 160, 320, 640),
            "conv_mlp": True
        },
    ...
    "convnext_tiny": {
        ...
        "params": {
            "out_channels": (3, 48, 96, 96, 192, 384, 768),
            "depths": (3, 3, 9, 3),
            "dims": (96, 192, 384, 768),
            "use_checkpoint": True
        },
    ...
    "convnext_small": {
        ...
        "params": {
            "out_channels": (3, 24, 48, 96, 192, 384, 768),
            "depths": (3, 3, 27, 3),
            "dims": (96, 192, 384, 768),
        },
    },
    "convnext_base": {
        ...
        "params": {
            "out_channels": (3, 32, 64, 128, 256, 512, 1024),
            "depths": (3, 3, 27, 3),
    ...
    "convnext_large": {
        ...
        "params": {
            "out_channels": (3, 48, 96, 192, 384, 768, 1536),
            "depths": (3, 3, 27, 3),
