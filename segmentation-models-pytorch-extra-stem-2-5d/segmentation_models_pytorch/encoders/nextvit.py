...
class NextViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0,
                 ...
                 use_checkpoint=False, with_extra_norm=True, frozen_stages=-1,
                 norm_eval=False,
                 ):
        ...
        if with_extra_norm:
            ...
            for stage_id in range(len(self.stage_out_channels)-1):
...
class NextViTEncoder(NextViT, EncoderMixin):
    ...
    def forward(self, x):
        features = [x]
        features = features + super().forward(x)
        features[1] = F.interpolate(features[1], scale_factor=2, mode="bilinear")
        # [2, 4, 8, 16, 32]
        return features
...
nextvit_weights = {
    "nextvit_small": {
        "imagenet": "nextvit_small_in1k6m_384.pth",  # noqa
    },
    "nextvit_base": {
        "imagenet": "",  # noqa
    },
...
pretrained_settings = {}
for model_name, sources in nextvit_weights.items():
...
nextvit_encoders = {
    "nextvit_small": {
        "encoder": NextViTEncoder,
        "pretrained_settings": pretrained_settings["nextvit_small"],
        "params": {
            "out_channels": (3, 64, 96, 256, 512, 1024),
            "stem_chs": [64, 32, 64],
            "depths": (3, 4, 10, 3),
            "path_dropout": 0.2,
            "use_checkpoint": True,
    ...
    "nextvit_base": {
        "encoder": NextViTEncoder,
        "pretrained_settings": pretrained_settings["nextvit_base"],
        "params": {
            "out_channels": (3, 64, 96, 192, 384, 768),
            "stem_chs": [64, 32, 64],
            "depths": (3, 4, 20, 3),
            "path_dropout": 0.2,
            "use_checkpoint": True,
    ...
    "nextvit_large": {
        "encoder": NextViTEncoder,
        "pretrained_settings": pretrained_settings["nextvit_large"],
        "params": {
